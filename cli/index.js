...
const run = async () => {
  ...
  while (moreStamps) {
    ...
    const rate = new BigNumber(addedStamp.rate).times(
      10 ** addedStamp.tokenDetails.decimals
    )
    stamps.push({
  ...
  fs.writeFile('../config/server-settings/server-config.json', JSON.stringify(configObj, null, 2), err =>
